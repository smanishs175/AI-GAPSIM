    ...
    async with async_session() as db:
        ...
        for i in range(50):
            ...
            substation = Substation(
                ...
                geometry=f"POINT({random.uniform(-125, -100)} {random.uniform(30, 45)})",
                metadata_json={"owner": f"Utility {i%10 + 1}", "ba_id": ba.id}
            )
            db.add(substation)
        ...
        for i in range(200):
            ...
            bus = Bus(
                ...
                bus_type=random.choice([1, 2, 3]),  # 1=PQ, 2=PV, 3=Slack
                geometry=f"POINT({random.uniform(-125, -100)} {random.uniform(30, 45)})",
                metadata_json={"type": random.choice(["PQ", "PV", "Slack"]), "substation_id": substation.id, "ba_id": substation.metadata_json["ba_id"]}
            )
            db.add(bus)
            buses.append(bus)
        ...
        for i in range(100):
            ...
            generator = Generator(
                ...
                bus_id=bus.id,
                gen_type=gen_type,
                p_gen=p_max * random.uniform(0.5, 0.9),
                q_gen=p_max * random.uniform(0.1, 0.2),
                p_max=p_max,
                p_min=p_max * 0.1,
                q_max=p_max * 0.3,
                q_min=-p_max * 0.3,
                geometry=f"POINT({random.uniform(-125, -100)} {random.uniform(30, 45)})",
                metadata_json={"fuel_cost": random.uniform(10, 50), "status": random.choice([True, True, True, False]), "ba_id": bus.metadata_json["ba_id"]}
            )
            db.add(generator)
        ...
        for i in range(250):
            ...
            branch = Branch(
                ...
                to_bus_id=to_bus.id,
                rate1=random.uniform(100, 1000),
                rate2=random.uniform(110, 1100),
                rate3=random.uniform(120, 1200),
